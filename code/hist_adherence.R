...
temp = 458  # temp=max(tot_real$newFreq)
prova=rep(0,temp)
prova2=rep(0,temp)
prova3=rep(0,temp)
for (i in 1:temp) {
...
# used to save the hist_adherence.RData without tot or wlc
rm(tot_sim, tot_sim2, tot_real)
rm(wlc)

save.image("results/hist_adherence.RData")

require("processx")
withr::with_dir("results", orca(fig, "hist_adherence.png", width = 2114, height = 842))
